...
export const MoltenDB = (options) => {
     ...
     */
    const storageConnection = (storage) => {
     ...
     */
    const getCollectionOptions = (name) => {
     ...
     */
    const storeCollectionOptions = (collectionOptions) => {
        return storageConnection(options.collectionsStorage.storage).then((connection) => {
            return connection.getStore(collectionsStoreOptions);
        }).then((store) => {
     ...
     */
    const createStoreOptions = (collectionOptions, storageKey) => {
        ...
        if (Object.keys(fields).length) {
            return __assign({}, storage.options, { name: storage.name, fields });
        }
    };
    /**
     ...
     */
    const createCollectionInstance = (collectionOptions) => {
    ...
    const createResultInstance = (collectionOptions, item) => {
        let resultInstance = {};
    ...
    return storageConnection(options.collectionsStorage.storage).then((connection) => {
        return connection.checkStore(collectionsStoreOptions).then((checkResult) => {
            ...
            return {
                ...
                deleteCollection: (name, deleteStores) => {
                    return getCollectionOptions(name).then((collectionOptions) => {
                        if (typeof collectionOptions === 'undefined') {
